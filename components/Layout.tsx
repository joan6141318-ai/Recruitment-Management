  ...
  const NavItem = ({ to, icon: Icon, label }: { to: string, icon: any, label: string }) => {
    ...
    return (
      <Link
        ...
        className={`flex items-center justify-between px-3 py-2.5 mx-2 rounded-md transition-all duration-200 group ${
          active 
      ...
      >
        <div className="flex items-center gap-3">
            <Icon size={18} strokeWidth={active ? 2.5 : 2} className={active ? 'text-white' : 'text-gray-400 group-hover:text-gray-900'} />
    ...
    <div className="h-[100dvh] bg-gray-50 flex flex-col md:flex-row overflow-hidden font-sans text-sm">
      ...
      <div className="md:hidden bg-white border-b border-gray-200 px-4 py-3 flex justify-between items-center z-30 shadow-sm">
        <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-black rounded flex items-center justify-center"><Moon size={14} className="text-white fill-white" /></div>
            <span className="font-bold text-sm tracking-tight text-gray-900">Agencia Moon</span>
        </div>
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 bg-gray-50 rounded-md border border-gray-200 text-gray-700 active:scale-95 transition-transform">
      ...
      {/* Sidebar Desktop */}
      <aside className={`
        fixed md:static top-0 left-0 h-full w-64 bg-white border-r border-gray-200 z-40 transform transition-transform duration-300 ease-in-out flex flex-col shadow-xl md:shadow-none
      ...
      `}>
          ...
          <div className="h-16 flex items-center px-6 border-b border-gray-100">
             <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                    <h1 className="font-bold text-sm text-gray-900 leading-none tracking-tight">Agencia Moon</h1>
                    <span className="text-[10px] text-gray-400 font-medium mt-0.5 uppercase tracking-wider">Management</span>
                </div>
             </div>
          </div>
          ...
          <nav className="flex-1 py-6 space-y-1">
              <div className="px-5 mb-2">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Menu Principal</p>
              </div>
          ...
          <div className="p-4 border-t border-gray-100 bg-gray-50">
             <div className="flex items-center justify-between mb-3">
                <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Usuario Conectado</p>
                    {isEditingName ? (
                ...
                <div className="px-2 py-0.5 rounded bg-gray-200 text-[9px] font-bold text-gray-600 uppercase">{user.rol}</div>
             </div>
             
             <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-white border border-gray-200 hover:border-red-200 hover:bg-red-50 hover:text-red-600 rounded-md transition-all text-xs font-bold text-gray-600 shadow-sm">

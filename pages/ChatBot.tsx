...
const ChatBot: React.FC<ChatBotProps> = ({ user }) => {
  ...
  const buildSystemInstruction = () => {
    ...
    3. Si presentas datos numéricos, alinéalos de forma que parezca una lista profesional limpia.
    4. Sé extremadamente amable, servicial y profesional.
    5. No utilices Markdown para negritas o cursivas. Usa mayúsculas solo para encabezados cortos.
    6. Tus respuestas deben ser visualmente entendibles y estéticas sin símbolos basura.`;
  };
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          temperature: 0.3, // Menos aleatoriedad para mayor orden
        },
      });

      // Limpieza extra de cualquier símbolo que el modelo intente colar
      const cleanedText = (response.text || "").replace(/[*_#-]/g, '');

      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        type: 'bot',
        text: cleanedText || "No se pudo procesar la consulta en este momento.",
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }]);
    } catch (error) {
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        type: 'bot',
        text: "He tenido un inconveniente técnico al procesar su solicitud. Por favor, inténtelo de nuevo.",
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        isError: true
      }]);
    } finally {

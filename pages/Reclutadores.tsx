...
import { UserPlus, Calendar, Trophy, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
       ...
       <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900 uppercase tracking-tight">Equipo de Reclutamiento</h2>
            <div className="flex items-center gap-2 mt-1">
                <Calendar size={12} className="text-gray-400"/>
                <span className="text-xs font-medium text-gray-500 capitalize">{monthName}</span>
                <span className="text-xs text-gray-300">|</span>
                <span className="text-xs font-bold text-black uppercase">Meta Individual: {GOAL_RECRUITMENT} Emisores</span>
            </div>
          </div>
          <button onClick={() => setIsModalOpen(true)} className="bg-black text-white px-4 py-2 rounded-lg text-xs font-bold uppercase flex items-center gap-2 hover:bg-gray-800 shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50 text-xs">
               {recruiters.map((rec, index) => {
                  ...
                  return (
                     <tr key={rec.id} className={`hover:bg-gray-50 transition-colors ${isTop ? 'bg-yellow-50/30' : ''}`}>
                        ...
                        <td className="px-5 py-3 align-middle">
                           <div className="flex items-center gap-3">
                              <div className="flex-1 bg-gray-100 h-2 rounded-full overflow-hidden">
                                 <div 
                                    className={`h-full rounded-full ${progress >= 100 ? 'bg-green-500' : progress >= 50 ? 'bg-blue-500' : 'bg-black'}`} 
       ...
       {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
           <div className="bg-white rounded-lg w-full max-w-sm p-6 shadow-xl">
              <h3 className="text-sm font-bold uppercase mb-4 text-gray-900">Invitar Reclutador</h3>
              <form onSubmit={handleRegister} className="space-y-3">
                 <div>
                    <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Nombre</label>
                    <input required className="w-full border border-gray-300 p-2 rounded text-sm focus:border-black outline-none" placeholder="Nombre completo" value={newName} onChange={e => setNewName(e.target.value)} />
                 </div>
                 <div>
                    <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Correo</label>
                    <input required type="email" className="w-full border border-gray-300 p-2 rounded text-sm focus:border-black outline-none" placeholder="correo@ejemplo.com" value={newEmail} onChange={e => setNewEmail(e.target.value)} />
                 </div>
                 <div className="pt-2">
                    <button className="w-full bg-black text-white py-2.5 rounded text-xs font-bold uppercase hover:bg-gray-800 transition-colors">Enviar Invitación</button>
                    <button type="button" onClick={() => setIsModalOpen(false)} className="w-full text-[10px] text-gray-400 font-medium underline mt-3 text-center block">Cancelar</button>
                 </div>
              </form>
           </div>
        </div>

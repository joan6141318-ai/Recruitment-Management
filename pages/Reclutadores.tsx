...
import { UserPlus, X, Mail, ShieldCheck, AlertTriangle } from 'lucide-react';
...
const Reclutadores: React.FC<ReclutadoresProps> = ({ user }) => {
  ...
  const handleToggleAccess = async (rec: User) => {
    ...
    if (confirm(`¿Estás seguro de ${action} el acceso a ${rec.nombre}?`)) {
  ...
  return (
    <div className="pb-8">
      <div className="flex justify-between items-center mb-6 pt-4">
          ...
          <button onClick={() => setIsInviteOpen(true)} className="bg-black text-white px-5 py-3 rounded-xl text-xs font-bold hover:bg-gray-800 transition-colors flex items-center gap-2 shadow-lg">
      ...
      <div className="grid gap-4">
          ...
          {recruiters.map(rec => {
              ...
              return (
                  <div key={rec.id} className={`bg-white p-5 rounded-2xl border transition-all ${isBanned ? 'border-accent/30 bg-orange-50/10' : 'border-gray-100 shadow-card'}`}>
                      <div className="flex justify-between items-start mb-4">
                          ...
                          <div className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wide flex items-center gap-1 ${isBanned ? 'bg-accent/10 text-accent' : 'bg-green-100 text-green-700'}`}>
                              {isBanned ? (
                                  <><AlertTriangle size={10} /> Revocado</>
                              ) : (
                      ...
                      <button 
                        ...
                        className={`w-full py-3 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all ${
                            ...
                            : 'bg-white border border-gray-200 text-accent hover:bg-accent hover:text-white'
                        }`}
                      >
                          {isBanned ? 'Reactivar Acceso' : 'Revocar Permisos'}
                      </button>
                  </div>
              );
          })}

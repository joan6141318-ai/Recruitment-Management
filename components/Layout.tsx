...
const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      {/* Sidebar / Drawer */}
      <aside className={`
        fixed md:sticky top-0 left-0 h-screen w-64 bg-white shadow-xl md:shadow-none z-30 transform transition-transform duration-300 ease-in-out
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around p-3 z-20 pb-safe">
        ...
        <Link to="/emisores" className={`flex flex-col items-center ${isActive('/emisores') ? 'text-primary' : 'text-gray-400'}`}>
          <Radio size={24} />
          <span className="text-[10px] mt-1">Emisores</span>
        </Link>
         {user.rol === 'admin' ? (
